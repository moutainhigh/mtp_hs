...
public class ClientMsgServiceImpl implements ClientMsgService{
	...
	public void doAcctReq(Msg10702 msg10702, Long recMsgID) throws Exception {
		try{
			...
			if(Integer.parseInt(msg10702.getChangeType()) == 2)
				client = clientDao.getClientMapper().selectByTradeAcct(msg10702.getTradeAcct());
			...
			if (Integer.parseInt(msg10702.getChangeType()) == 1) {
				...
				json.put("tradeAccount", msg10702.getTradeAcct());		//会员编号
				json.put("memberType", (msg10702.getClientAcctType()==0 ? 8:msg10702.getClientAcctType())+"");
				// 会员类型Y 1-交易所，2-综合类，3-结算类，4-经纪类；5-交易类；6-产业类；7-服务类；8-其他
			...
			json.put("exchangeMemberStatus", "2");    //会员状态：1-已签约；2-未签约；3-销户；默认为签约
			...
			json.put("idNo", msg10702.getCertCode());
			json.put("gender", CenterToExchUtil.HS_SEX.get(msg10702.getSex()));
			json.put("nationality", msg10702.getNationality());
			json.put("businessCert", (Integer.parseInt(msg10702.getCertType()) == 4 ? msg10702.getCertCode() : 0)+""); // 工商营业执照N
			json.put("orgCode", msg10702.getClientType()==2 ? (StringUtil.isEmpty(msg10702.getOrgCode()) ? "0" :msg10702.getOrgCode()) : msg10702.getOrgCode());
					/*msg10702.getOrgCode()==null?"0":msg10702.getOrgCode());  StringUtil.isEmpty(msg10702.getOrgCode()) ? "0" :msg10702.getOrgCode()*/
			json.put("taxCert", "0"); // 税务登记证号
			json.put("taxCertType", "1"); // 税务登记证类型
...
//			}
			System.out.println(errorNo+errorInfo);
...
//			msg10703.setBankSeq(clientRecId + "");
			...
			if (errorNo == 0){
				...
				clientRec.setDealStatus(FinalConstants.DealStatus.SUCCESS);
			}
			else if (errorNo == 99) {
				msg10703.setRetCode(FinalConstants.RetCode.UNKNOWN);
				msg10703.setRetDesc(errorInfo);
				clientRec.setDealStatus(FinalConstants.DealStatus.UNKNOW);
			} else {
				msg10703.setRetCode(FinalConstants.RetCode.FAIL);
				msg10703.setRetDesc(errorInfo);
				clientRec.setDealStatus(FinalConstants.DealStatus.FAIL);
			}
			LOGGER.info("*记录往包*");
			...
			clientRec.setDealDesc(errorInfo);
			clientRecDao.getClientRecMapper().updateByPrimaryKeySelective(clientRec);
			if(errorNo != 0&&errorNo != 99){

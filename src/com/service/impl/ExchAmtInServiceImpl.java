...
public class ExchAmtInServiceImpl implements ExchAmtInService {
	...
	public void doAmtInReq(Msg10102 msg10102, Long recMsgID) throws Exception {
		try{
			...
			Account account = accountMapper.selectByTradeAcct(msg10102.getTradeAcct());
			LOGGER.info("*插入出入金记录*");
			Tran tran = tranMapper.selectByPrimaryKey(msg10102.getTranNo());
...
//			ukey证书	uKeyStr
			...
			if (errorNo == 0){
				amtRec.setDealDesc(errorInfo);
				amtRec.setDealStatus(FinalConstants.DealStatus.SUCCESS);
				...
				amtRec.setDealStatus(FinalConstants.DealStatus.FAIL);
				amtRec.setDealDesc(errorInfo);
				msg10103.setRetCode(FinalConstants.RetCode.FAIL);
				msg10103.setRetDesc(errorInfo);
			}
			amtRecDao.getAmtRecMapper().updateByPrimaryKeySelective(amtRec);
			LOGGER.info("*记录往包*");
